## Methods - External Gene-Set Validation
...
## Results - External Gene-Set Recovery

External gene-set recovery provided an independent biological validation of the model rankings. Across top-N thresholds, the analysis measured whether externally supported T2D-relevant genes appeared more often in the top-ranked lists than expected under random sampling. The strongest Top-100 enrichment was observed for Mitochondrial_OXPHOS_complex_I in the DNABERT-2 multimodal model, with 4 observed genes compared with a random expectation of 1.92 genes. These results provide a complementary validation layer to the internal performance metrics by assessing whether model rankings recover independently supported biological knowledge.

## Discussion - External Validation Interpretation
